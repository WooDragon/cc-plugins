...
def run_worker(alias, model_id, client, config, search_backends, fetch_backends,
    ...
    try:
        ...
        for _ in range(max_steps):
            ...
            if tool_calls:
                is_all_fetch = all(tc.get("function", {}).get("name") == "fetch" for tc in tool_calls)
                if len(tool_calls) > 1 and is_all_fetch:
                    for tc, result_text in zip(tool_calls, _run_fetch_calls_parallel(
                            tool_calls, search_backends, fetch_backends, journal, config, local_dir)):
