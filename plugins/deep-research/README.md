...
The framework treats the main session as a **Lead** that orchestrates the pipeline: it dispatches采集 to `research-harvester`, decomposition/synthesis to `research-analyst`, sufficiency/quality review to `research-reviewer`, and optional visual `<!-- ds:xxx -->` annotation to `research-publisher`, gating each stage transition (G0–G3). Final report.md → report.html rendering is a deterministic transform executed by the plugin's built-in `scripts/render.py` (stdlib-only, byte-identical on repeated renders) — not generated by an agent. A `SubagentStop` hook mechanically verifies any `GATE_VERDICT: G1 PASS` claim against the harvest engine's citation journal — fail-open on anything uncertain, block only on a proven false PASS.
...
│   └── research-publisher.md   # Stage 7 (optional): visual <!-- ds:xxx --> annotation suggester, no HTML output
├── scripts/
...
│   ├── render.py / render_common.py / render_inline.py / render_blocks.py
│   │                            # deterministic report.md -> report.html renderer (zero timestamps/randomness)
│   ├── _sanitize.py             # shared anti-injection text cleaner (zero-width/bidi/quote-spoof), used by render.py + harvest.py
