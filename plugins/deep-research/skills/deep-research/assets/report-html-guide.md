...
### 分级硬约束：高信息密度内容默认组件化，散文才是 fallback

以下映射按约束强度分三档。**反转默认值**——不是"拿不准就退回朴素"，而是"内容一旦命中 MUST 信号，就不允许退化成 bullet list"：

- **必须组件化（MUST，命中即强制，不允许退化成 `ul`/`p`）**：
  - N 方案对比矩阵 → signal 染色表（推荐行加 `tr.rec`）
  - 分层建议 / 阶段路线图 / 短中长期规划 → `.phase-timeline`
  - 数据/维度就绪度盘点 → `.data-grid`
  - 并列问题/盲区/痛点清单 → `.pain-list`
- **应当考虑（SHOULD，命中信号后优先用组件，除非组件明显不服务理解）**：
  - 并列风险/tradeoff/要点 → `.card-grid`
  - 关键提示/核心矛盾 → `.callout`
  - ASCII 架构图 → `.arch-diagram`
  - 内联状态词 → `.tag`
- **兜底（fallback，唯一允许落到朴素文本的情形）**：确实不满足以上任何组件语义的普通叙述段落，才用 `section p` / `ul`。

组件是为了让高信息密度的内容形态更易读，不是为了给每一段都找个盒子装——但**高信息密度内容默认组件化，散文段落才是例外，不是相反**。见 §⑤.5 的黄金标准范例，感受"该组件化就必须组件化"落地后的效果密度。
...
- **Chanel 原则（出门前减一件配饰）**：克制。删掉不服务内容理解的装饰性组件——如果一段内容用 `section p` 就说得清楚，不要为了"好看"硬套 card-grid 或 tag
  - **边界（克制不是退化的挡箭牌）**：克制**只针对装饰性冗余**——不服务内容理解的纯装饰组件、重复的 tag、无意义的编号。**不得以"克制"为由，把本该用染色表/card-grid/phase-timeline 承载的高密度内容（见 §② MUST 分级）退化成 bullet 墙**。删装饰 ≠ 删结构：装饰是可以省的"配饰"，MUST 组件承载的是内容本身的结构语义，省掉它不是克制，是信息损失。

---

## ⑥ Gold-standard 渲染范例

这是高组件密度的目标形态。publisher 渲染时应对标这个密度，而不是退回文本墙——下面这段合成范例演示 §① 词汇表里的组件如何协同构成一份丰富的报告：`verdict-bar` + signal 染色表（含 `tr.rec`）+ `card-grid` + `phase-timeline` + `data-grid` + `callout` + `arch-diagram`，每种至少一处。
...
**这就是目标密度**：一份报告里，结论用 verdict-bar，对比用染色表，风险用 card-grid，路线图用 phase-timeline，数据盘点用 data-grid，架构用 arch-diagram，核心矛盾用 callout——散文段落只出现在这些组件都不适用的边角内容里，不是主体。
