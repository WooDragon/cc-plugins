...
6. **Symlinked instruction files are now scanned, with boundary validation.** `_gate_instruction_files` includes symlinks (`find -type f -o -type l`) and filters them through `_gate_in_tree`: a symlink whose resolved physical path falls outside the cwd tree is skipped (preventing scan escape, e.g. `CLAUDE.md -> /etc/passwd`). Dangling symlinks are also skipped.
