...
1. 问用户选哪套（或基于内容推荐一套）
2. 在 `DeckLayout` 组件传入 `theme` prop：
   ```astro
   <DeckLayout title="..." theme="ink-classic">
   ```
   或
   ```astro
   <DeckLayout title="..." theme="indigo-porcelain">
   ```
3. CSS 变量（`--ink`、`--paper` 等）由 DeckLayout 自动注入，无需手动修改

**当前 Astro 已实现的主题**：`ink-classic`（墨水经典）、`indigo-porcelain`（靛蓝瓷）

**尚未实现**：🌿 森林墨、🍂 牛皮纸、🌙 沙丘——需要先添加对应 GLSL shader 文件到 `src/shaders/` 并扩展 DeckLayout
