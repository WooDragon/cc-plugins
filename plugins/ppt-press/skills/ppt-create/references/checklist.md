...
### 0. 生成前必须确认使用了正确的 Astro 组件(最重要)

**现象**：直接写原始 HTML 而非 Astro 组件，结果样式全部丢失——大标题变成非衬线、数据大字报字体小得像正文、pipeline 多页糊成一坨、图片堆到浏览器底部。

**根因**：Astro 组件架构下，样式类名由组件内部定义。使用原始 HTML 标签而非 Astro 组件时，浏览器 fallback 到默认样式。

**做法**：
- **生成 PPT 前，确认使用 Astro 组件而非原始 HTML**——`<Slide>` 而非 `<section>`、`<StatCard>` 而非手写 `<div class="stat-card">`、`<Pipeline>` 而非 `<div class="pipeline">`
- **核对 `references/components.md` 的 Props**——每个组件的必填/可选 Props 都已列出，传错 prop 不会报错但会丢失样式
- 常用组件清单：`Slide / Chrome / Foot / Grid / StatCard / Pipeline / PipelineStep / Pillar / Callout / Rowline / OptionCard / FeatureCard / FrameImg / QuoteBlock`
- 如果需要项目中尚未定义的样式，**在项目的 `src/styles/` 对应 CSS 文件中补上**，不要在每页 inline 重写
- 生成后打开浏览器，如果看到"大标题是非衬线"或"pipeline 步骤挤在一行"，首先检查是否遗漏了组件 import 或写成了原始 HTML
...
- **含中文大标题额外注意行高**:`.h-hero` / `.h-xl` 手动设 inline `line-height` 时**不得 < 1.0**。CJK 字形填满整个 `em` 方框,行高小于字号时行盒兜不住字身,墨迹从下缘溢出、贴上正文;纯拉丁字形不填满 em 故不暴露(这就是贴脸只在中文标题上出现的原因)。诡异点:此时量相邻元素间距是正数,常规检测抓不到,只有像素级截图看得见。想紧凑用 `letter-spacing` 收字距或减小 `font-size`,别压行高
...
```
预检(生成前)
  □ 已确认所有 slide 使用 Astro 组件（非原始 HTML），Props 符合 references/components.md
