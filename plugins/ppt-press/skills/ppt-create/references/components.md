...
这是 `guizang-ppt-skill` skill 的组件使用手册。Astro 组件 Props 的完整定义见仓库 `docs/components.md`，本文档聚焦于"这个组件长什么样、什么时候用、有哪些常见变体"。
...
每一页是一个 `<Slide>` 组件，`theme` 必填，其他 props 按需。

```astro
<Slide theme="light">            <!-- 浅色页（默认 cascade 动画） -->
<Slide theme="dark">             <!-- 深色页 -->
<Slide theme="light" hero>       <!-- Hero 页：浅色 + 薄遮罩，WebGL 透出 -->
<Slide theme="dark" hero>        <!-- Hero 页：深色 -->
<Slide theme="light" animate="quote">   <!-- 显式指定动画 recipe -->
```

**light vs dark 的使用：交替使用**，每 2-3 页切换一次主题，避免连续超过 3 页同色。翻页时 WebGL 背景会自动在两个 shader 之间渐变过渡。

**hero prop 的使用**：只给视觉主导的页面加（封面、金句页、章节过渡、结尾）。加 `hero` 后遮罩降到 12-16%，WebGL 背景大幅透出，不要在 hero 页放太多文字。

**附加 class 或 style**：
```astro
<Slide theme="dark" class="compact-stats">  <!-- 追加自定义 class -->
<Slide theme="light" style="--override:value">
```
...
每一页的顶部和底部元信息条。几乎所有页都应该有。

```astro
<Chrome left="第一幕 · 硬数据" right="02 / 27" />

{/* ... 页面主体 ... */}

<Foot title="项目名 · CodePilot" meta="Act I · Dev Numbers" />
```

**规则**：
- `Chrome right` 总是放页码 `NN / TOTAL`
- `Foot title` 是主说明（中英文均可），`meta` 是右侧补充
- Chrome 和 Foot 共同构成杂志感的"页眉页脚"，位置固定在 Slide 顶/底

---

## Callout 引用框

展示金句 / 关键观点 / 他人引言。自带 `data-anim`，无需额外标记。
...
变体：
- 不带来源：省略 `source` prop 即可
- 带英文金句：slot 里用 `<em class="en">"Thin Harness, Fat Skills."</em>`
- 自定义宽度：`<Callout style="max-width:80vw">`
- 在 hero 页使用：外层 wrap 一个 `<div style="position:relative;z-index:2">`（避免被背景遮罩盖住）

---

## Stat 数字矩阵

展示数据指标，用 `<StatCard>` 包在 `<Grid>` 里。自带 `data-anim`。

```astro
<Grid cols="6" style="margin-top:5vh">
  <StatCard label="Duration"  value="64"    unit="天"  note="从 0 到现在" />
  <StatCard label="Stars"     value="5,166"             note="一个开源仓库" />
  <StatCard label="Downloads" value="41K+"              note="装到了几万台电脑里" />
</Grid>
```

**常用 `cols` 值**：
- `cols="6"` — 3×2 网格（最常用，6 个 stat）
- `cols="4"` — 2×2 网格（4 个 stat）
- `cols="3"` — 3 等分单行（3 个 stat）

`unit` prop 比 `value` 字号小，视觉上像上角标；`note` 是最小字号说明文字。数字建议 2-4 字符（太长会溢出），用 K / M 简写。
...
列表式内容，每行一个条目。自带 `data-anim`。

```astro
<Rowline k="CLAUDE.md" v="你该怎么做事 —— 行为规则 + 工作偏好 + 禁止事项" m="EMPLOYEE · HANDBOOK" />
<Rowline k="agents.md" v="当前任务分解与 agent 协作上下文" />
```

三列：`k` 衬线关键词 · `v` 正文描述 · `m` 等宽标签（右对齐，可省略）。第一个和最后一个 Rowline 自动加上下边框。

---

## Pillar 支柱卡

三支柱结构，常用于"概念并列"类型页面。自带 `data-anim`。

```astro
{/* 数字序号版 */}
<Grid cols="3">
  <Pillar number="01" title="三层文档体系" desc="CLAUDE.md + 项目知识库 + 护栏文件" />
  <Pillar number="02" title="判断力"       desc="决策和方向的权威。取舍、品味、方向感。" />
  <Pillar number="03" title="执行力"       desc="快速从想法到可运行代码。" />
</Grid>

{/* Lucide 图标版 */}
<Grid cols="3">
  <Pillar icon='<i data-lucide="compass"></i>'    title="判断力" desc="决策和方向的权威。" />
  <Pillar icon='<i data-lucide="zap"></i>'        title="执行力" desc="快速从想法到代码。" />
  <Pillar icon='<i data-lucide="shield-check"></i>' title="掌控力" desc="护栏和文档体系。" />
</Grid>
```

`number` 和 `icon` 二选一（都不填则只显示 title + desc）。图标用 Lucide CDN，名称参见下方 Icons 章节。
...
**严禁使用 emoji**。用 Lucide via CDN（DeckLayout 已引入）。
...
整套 deck 默认开启翻页入场动画，由 Motion One 驱动。DeckLayout 已包含完整初始化，无需改动 JS。

### 加载方式

`src/scripts/animation.ts` 先尝试本地 `/assets/motion.min.js`，失败回落 jsdelivr CDN，两者都失败则强制把所有 `data-anim` 元素设为 `opacity:1`——内容永远可读，演示不依赖网络。
...
```astro
{/* 1. 在 Slide 上选 recipe（hero 自动用 hero recipe，其他默认 cascade） */}
<Slide theme="light" animate="quote">

{/* 2. 在需要入场的元素上加 data-anim（可选值：left/right/line/step） */}
...
- **内容静态时也不显示?** 检查 `public/assets/motion.min.js` 是否存在；或者浏览器控制台看错误信息（必须通过 `npm run dev` 访问，不能直接 `open .html`）
