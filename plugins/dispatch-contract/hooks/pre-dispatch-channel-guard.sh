...
printf '[dispatch-channel-guard] mailbox 到达时刻绑定派发方空闲：SendMessage 不是即时投递，派发方主循环有工具在飞时消息会积压，直到派发方停下来才 flush，实测曾见数分钟级积压，时长不可预测。teammate 通道因此不适合"拿到产物才能往下走"的同步决策——延迟到决策点之后等价于丢失；需要即时产物就走出路①。\n' >&2
