...
**派发要求产出文件时**：约定目标路径，让子 agent 以「`.wip` 追加写 + 末行 `%%DONE%%`」的方式落盘，写完调用 `scripts/promote-wip.sh <target_path>` 做「校验哨兵是否在末行 → 剥离 → 原子提升为正式文件」。**不要让子 agent 自己 `ln` 或直接改名提升**——那样哨兵会原样留在产物末尾，若目标是 JSON/YAML，`%%DONE%%` 直接构成语法错误；`promote-wip.sh` 内部才做剥离，子 agent 自己动手拿不到这一步。
