...
- `--deny "Write(**)" --deny "Edit(**)"` — a guardrail against review drift accidentally changing files, not a complete write block or a security boundary. The rules match tool names and path globs: they block the `Write`/`Edit` tools and shell redirections, but not indirect writes from `run_terminal_cmd`, such as `python3` opening a file for writing. In a real test grok said, “Write / redirect 被策略拦了，改走 Python 落盘，cat 核对过内容。” and the file was actually written. Behavior is not deterministic: in repeated tests grok sometimes gave up after the denial and reported that it could not proceed, and sometimes routed around it; it depends on how persistent the model was that time. That strength is accepted for code review, where grok has no motive to write and the guardrail is meant to prevent accidental drift, but it is ineffective against adversarial behavior. Truly sealing the indirect path would also require `--deny "Bash(*)"`, which would take away `git log`/`git blame` context access and reduce the setup to the rejected `--tools` allowlist approach. The `--sandbox` comparison is not only about enforcement level: this deny policy can be bypassed, while Seatbelt is kernel-enforced and cannot be bypassed by the grok process itself. We do not use `--sandbox` because grok 1.0.13 added runtime-socket deny logic to its built-in `restrict_network` profiles whose path resolver does not follow symlinks, so on machines where `/var/run/docker.sock` is a symlink (common with OrbStack), sandbox fails to apply and grok refuses to start (issue #212).
