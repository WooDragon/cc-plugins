...
3. `SID=$(uuidgen)`（或兜底方案，见下），`grok --prompt-file <tmp> -m <model> --effort <effort> --deny "Write(**)" --deny "Edit(**)" --deny "Bash(git -C:*)" --deny "Bash(git <每个写子命令>:*)" --output-format plain -s "$SID"` 产出评审，打印到终端。
...
- **写保护 `DENY_WRITE_FLAGS`**：`--deny "Write(**)"`、`--deny "Edit(**)"`、`--deny "Bash(git -C:*)"`，外加每个 git 写子命令一条 `--deny "Bash(git <sub>:*)"`（checkout/switch/branch/symbolic-ref/update-ref/reset/revert/merge/rebase/cherry-pick/am/apply/stash/worktree/bisect/restore/clean/add/rm/mv/commit/tag/push/fetch/pull/clone/remote/submodule/init/config/gc/reflog/sparse-checkout/update-index）。这是防评审跑偏时顺手改文件或改分支的护栏，不是完整的写入封锁，也不是安全边界。规则按工具名、命令前缀和路径 glob 匹配，能拦住 `Write`/`Edit` 工具、shell 重定向，以及上列 git 写子命令。**为何加 git 子命令**：只 deny `Write`/`Edit` 时，grok 在一次真实评审中自行执行 `git checkout` 切走了调用方仓库的分支——写保护只盯落盘，漏了工作树与 ref 状态（#221）。**为何连 `git -C` 一起封**：前缀匹配下 `git -C . checkout -b probe` 不命中 `Bash(git checkout:*)`，实测直接放行；代价是 `git -C <path> log` 这类读形态也被挡，而评审已由 `--cwd` 定位仓库、不需要该形态。**强度**（grok 1.0.13 实测）：常态有效——只 deny `checkout`/`switch` 时，grok 被拦后老实报告失败、不主动绕；对抗无效——在 44 条 git 写黑名单下，明示要求它换等价方式，它用 `python3` 直写 `.git/HEAD` 与 `.git/refs/heads/<branch>` 完成了切分支，落盘侧同样能经 `python3` 绕开（grok 原话："Write / redirect 被策略拦了，改走 Python 落盘，cat 核对过内容。"）。根因是锚点错位：deny 匹配「工具名 + 命令前缀 + 路径 glob」，而 shell 是通用执行器，任何写都能换个前缀重新表达；仍敞着的口子包括 `git --git-dir=`、`env git`、`/usr/bin/git`、`sh -c` 与任何解释器直写。若再加 `--deny "Bash(*)"` 才能真正封住这类间接写入，但会同时拿走 grok 跑 `git log`/`git blame` 追上下文的能力，等于退化成已放弃的 `--tools` 白名单方案。与 `--sandbox` 的差距不只是层级不同：当前 deny 策略能被间接写入绕过，而 Seatbelt 是内核强制、grok 进程本身无法绕过。为何不用 `--sandbox`：grok 1.0.13 给带 `restrict_network` 的内置 profile（`read-only`/`strict`）新增了容器 runtime socket（如 `/var/run/docker.sock`）deny 逻辑，其路径解析器不跟随 symlink——在该路径为 symlink 的机器（常见于 OrbStack）上，sandbox 整个无法应用、grok 直接拒绝启动（见 issue #212）；这是 grok 自身的上游回归，此前长期正常工作。真正的隔离应走容器化（一次性 clone + `--cwd`），本脚本不覆盖。

**连带代价**：`branch`/`remote`/`config`/`stash`/`tag`/`reflog`/`submodule`/`worktree` 是读写一体命令，整条收进名单后它们的只读 listing 形态（`git branch`、`git remote -v`、`git config --get`、`git stash list`）也被挡住；评审要查分支状态改用 `git status` 与 `git rev-parse`，这两条不在名单内。

- **grok 继承 `~/.claude/settings.json` 的权限规则**：`grok inspect` 的 Permissions 段显示它直接加载该文件（本机实测 46 条）。这意味着用户给 Claude Code 的授权会原样进入评审进程——例如其中的 `"Bash(git:*)"` allow 就是「只传 `--allow` 不传 `--deny` 时未匹配命令仍被放行」的根因。CLI 传入的 `--deny` 优先级高于 settings 里的 allow，故 `DENY_WRITE_FLAGS` 能压住继承来的 allow。
